...
if(isset($_POST['nameCall']) && (isset($_POST['phoneCall']) || isset($_POST['emailCall']))) {
    ...
    $result = $conn->Insert('notifications', [0 => $admin[0], 1 => $_SESSION['id'], 2 => 'Будь-ласка, підтвердьте мій акаунт. Моє ім\'я '.$_POST['nameCall'].', контакти: '.$contacts, 3 => date('Y-m-d H:i:s'), 4 => 'New user'], [1 => 'MessageTo', 2 => 'MessageFrom', 3 => 'Text', 4 => 'Date', 5 => 'Tag']);
    if ($result) {
...
if(isset($_POST['action']) && isset($_POST['target'])) {
    ...
    if ($_POST['action'] === 'update' && isset($_POST['values']) && $_POST['target'] === 'employee') {
        $result = $conn->Update($_POST['target'], $_POST['values'], '`ID`=' . $_POST['ids']);
        if ($result)
